...
def one_hot(x, num_classes):
    idx = x.long() # 정수형으로 변경
    idx = idx.view(-1, 1) # A x 1 행렬로
    x_one_hot = torch.zeros(x.size()[0] * x.size()[1], num_classes) # (170*10, 10) 짜리 값이 0인 행렬
    x_one_hot.scatter_(1, idx, 1) # (1 열방향, 1700개의 값을 가짐, 1700행 동안 해당하는 열에 1값 넣어줌) 
    x_one_hot = x_one_hot.view(x.size()[0], x.size()[1], num_classes) # 170, 10, 25로 재변환
    return x_one_hot
...
class LSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        out, _ = self.lstm(x, (h_0, c_0)) # (hn, cn)은 필요 없으므로 _ 처리(out으로만 처리하면 tuple 처리됨. 그래서 , _ 처리 하면 그 데이터 안쓴다는 의미)
        # Note: the output tensor of LSTM in this case is a block with holes
        # > add .contiguous() to apply view()
        out = out.contiguous().view(-1, self.hidden_size) # Reshape to (batch * seq_len, hidden_size)
        # Return outputs applied to fully connected layer (linear하게 처리하기 위해서 fully connected 하게 만든 것)
        out = self.fc(out) # Class들의 Logit 반환
        return out
...
for epoch in range(num_epochs):
    ...
    _, idx = outputs.max(1) # 여기서 idx는 max값이 몇번째 열에 있나를 나타냄
    idx = idx.data.numpy()
